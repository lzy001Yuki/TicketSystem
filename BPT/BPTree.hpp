...
private:
    ...
    Yuki::HashMap<int, node, Function, CacheSize, 1303> Cache;
...
public:
    ...
    bool findKV(const Key &k, Value &obj) {
        int pos = keyBinarySearch(root.key, root.len, k);
        node nextNode;
        if (root.key[pos].first == k) pos++;
        int index = root.sonPos[pos];
        if (!Cache.find(index, nextNode)) readAndCache(nextNode, index);
        //readAndCache(nextNode, index);
        while (!nextNode.isLeaf) {
            int pos_ = keyBinarySearch(nextNode.key, nextNode.len, k);
            if (nextNode.key[pos_].first == k) {
                pos_++;
            }
            int index_ = nextNode.sonPos[pos_];
            if (!Cache.find(index_, nextNode)) readAndCache(nextNode, index_);
        ...
        int pos_ = keyBinarySearch(nextNode.key, nextNode.len, k);
        if (nextNode.key[pos_].first != k) return false;
        else {
