...
private:
    ...
    BPT<char, UserInfo, nameFunction, 20, 2, 8096> userData;
public:
    ...
    void LogTraverse() {
        BPT<char, UserInfo, nameFunction, 20, 2, 8096>::node u_root = userData.root;
        BPT<char, UserInfo, nameFunction, 20, 2, 8096>::node newNode = findMinIndex(u_root);
        while (true) {
    ...
    BPT<char, UserInfo, nameFunction, 20, 2, 8096>::node findMinIndex(const BPT<char, UserInfo, nameFunction, 20, 2, 8096>::node& tmp) {
        if (tmp.isLeaf) return tmp;
        int index = tmp.sonPos[0];
        BPT<char, UserInfo, nameFunction, 20, 2, 8096>::node newNode;
